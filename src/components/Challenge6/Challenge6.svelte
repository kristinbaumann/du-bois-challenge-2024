<script>
	...
	$: xBlackLeft1860 = innerWidth - xScale(xPercFact * (wSum - wBlack[1860]));
	$: xBlackLeft1840 = innerWidth - xScale(xPercFact * (wSum - wBlack[1840]));
	$: xBlackLeft1800 = innerWidth - xScale(xPercFact * (wSum - wBlack[1800]));
	// > brown
	$: xBrownLeft1890 =
	...
	// > light yellow
	$: xLightYellowLeft1890 = innerWidth - xScale(xPercFact * (wSum - wWhite - wLightYellow));
	// > white
	$: xWhiteLeft1890 = innerWidth - xScale(xPercFact * (wSum - wWhite));
	...
	function drawPathDarkYellow(passedTriangleTip) {
		const pa = path();
		pa.moveTo(passedTriangleTip.x, passedTriangleTip.y);
		pa.lineTo(passedTriangleTip.x, yScale(4));
		pa.lineTo(xLightYellowLeft1890, yScale(4));
		pa.closePath();
		return pa.toString();
	}
	// > light yellow clock wise
	function drawPathLightYellow(passedTriangleTip) {
		const pa = path();
		pa.moveTo(passedTriangleTip.x, passedTriangleTip.y);
		pa.lineTo(xLightYellowLeft1890, yScale(4));
	...
	function drawPathBlack(passedTriangleTip) {
		const pa = path();
		pa.moveTo(passedTriangleTip.x, yScale(1)); // not anymore: start at triangle tip
	...
	// > fuzzy line and area
	function drawFuzzyElements(passedXScale, passedTriangleTip) {
		let randomSet = [];
		const minX = xBlackLeft1800;
		const maxX = passedXScale(400000); // arbitrary value for fuzzy line ending
		const maxY = 1; // further above we have minY = 0.92
		const step = 1;
		let y;
		let counter = 1;
		let breakpoint = xDarkYellowLeft1890 - xBlackLeft1800; // breakpoint is the x value where the fuzzy line hits the triangle start
		...
		pa.lineTo(passedXScale(0), yScale(maxY + 0.05));
		...
		area.lineTo(passedTriangleTip.x, yScale(2));
	...
	$: ({ fuzzyLine, fuzzyArea } = drawFuzzyElements(xScale, triangleTip));
	$: blackPath = drawPathBlack(triangleTip);
	$: brownPath = drawPathBrown(triangleTip);
	$: darkYellowPath = drawPathDarkYellow(triangleTip);
	$: lightYellowPath = drawPathLightYellow(triangleTip);
</script>

<h2 class="headline">
	<p>The Amalgation of the White and Black elements of the population in the United States.</p>
...
<div class="chart-container" bind:clientWidth={width}>
	<svg {height}>
		<g transform="translate({margin.left}, {margin.top})">
			<g class="polygons">
				<path d={blackPath} fill="black" stroke="black" />
				<path d={fuzzyLine} fill="none" stroke="black" stroke-width="0.5" />
				<path d={fuzzyArea} fill="black" stroke="black" stroke-width="0.5" />
				<path d={brownPath} fill="#895c38" stroke="#895c38" />
				<path d={darkYellowPath} fill="#f4ad00" />
				<path d={lightYellowPath} fill="#e7ca87" />
			</g>
			...
			<g class="otherLabels">
				...
				<text
					x={triangleTip.x + (triangleTip.x - xScale(0) / 2)}
					y={yScale(3.5)}
					class="size-m black">Whites</text
				>
			</g>
		</g>
	</svg>
